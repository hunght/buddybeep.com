...
import { GenerateAnswerParams, Provider, ResponseContent } from './types'
import { APPSHORTNAME, ADAY, HALFHOUR } from '~app/utils/consts'
...
async function request_new(token: string, method: string, path: string, data?: unknown, callback?: unknown) {
  return fetch(`https://chat.openai.com/backend-api${path}`, {
    method,
    headers: {
      'Content-Type': 'application/json',
...
function deleteRecentConversations(token, data) {
  ...
  for (let i = 0; i < convs.length; i++) {
    ...
    if (HALFHOUR < now - conv_i_time && now - conv_i_time < ADAY && convs[i].title.indexOf(APPSHORTNAME + ':') != -1) {
      setTimeout(function () {
...
export class ChatGPTProvider implements Provider {
  constructor(private token: string) {
    this.token = token
    //Brute:
    request_new(token, 'GET', '/conversations?offset=0&limit=100&order=updated', undefined, deleteRecentConversations)
  }
